...
function getType(obj){
	...
	{	
		var myMask = new Ext.LoadMask(Ext.getBody(), {msg:"Please wait! Loading window..."});
    	myMask.show();
	...
	function compose(){
		alert('compose');
		// COMPOSE MAIL FORM
		var myMask = new Ext.LoadMask(Ext.getCmp('doc-body'), {msg:"Please wait! Loading window..."});
    	myMask.show();
...
//		
		...
		var store_search = new Ext.data.JsonStore({
			root: 'topics',
			totalProperty: 'totalCount',
			remoteSort: true,
			fields: [
			...
			proxy: new Ext.data.HttpProxy({
            			url: 'mail_search.jsp?id='+id+'&topic='+x
        			})
			});
		...
		var grid_search = new xg.GridPanel({
			store: store_search,
			layout: 'fit',
			loadMask: true,
			trackMouseOver:true,
	...
	//=======================
	  function share_mail(x,y){
		 ...
		 // alert('look@ SHARE OMS ID:'+z);
		  Ext.Ajax.request({
				url: 'share_email.jsp?oms_id='+z+'&email_id='+y+'&id='+id,
				success: function(r) {
					if(r.responseText=='timeOut')
						window.location = 'index.jsp';
					else{
