...
<%
	...
	try
	{
		...
		*/
		...
		sql = MailSqlGenerator.getRecipientByEmailId(email_id);
		...
		*/
		...
		sql = MailSqlGenerator.getAttachmentByEmailId(email_id);
		rs = stmt.executeQuery(sql);
		if(rs.next()){
				...
				rs = stmt.executeQuery(sql);
				while(rs.next()){
					clob_body=rs.getClob("content");
					if(clob_body!=null)
					attachment_loc=clob_body.getSubString(1,(int)clob_body.length());
					System.out.println("attachment_loc:::"+attachment_loc);
					System.out.println("clob_body.length():::"+clob_body.length());
					attachment=clob_body.getSubString(1,(int)clob_body.length());					
					output += "<tr><td width=\"3%\"><img src=\"images/mail/images/attachment-icon.png\" /></td>";
					output += "<td align=\"left\" class=\"t1\">";
					output += "<a href=\"mailinfoAction.csmp?method=downloadAttachment&file_name="+attachment_loc+"\" target=\"_blank\">"+attachment+"</a></td></tr>";
					}
				output += "</table><br>";
			}
		...
		*/
		...
		output_2 += "<tr>";
		output_2 +=	"<td width=\"30%\" class=\"t1\" align=\"left\" onclick=\"Ext.getCmp('mail_"+email_id+"').doLayout();show('share_"+email_id+"'); \" style=\"cursor:pointer;\">";
		output_2 +=	"<img src=\"images/mail/images/add-user-icon.png\" /> Share with other </td>";
		 ...
		 */
		sql = MailSqlGenerator.getReferenceByEmailId(email_id);
		System.out.println("SQL:>>>>>>>"+sql);
		rs = stmt.executeQuery(sql);
		if(rs.next()){
			if(rs.getString("refid")!=null) refId = rs.getString("refid");
			if(rs.getString("refdtl")!=null) ref_dtl = rs.getString("refdtl");
			if(rs.getString("emailgroupid")!=null) email_groupId = rs.getString("emailgroupid"); 
			if(rs.getString("groupname")!=null) email_group_name = rs.getString("groupname");
			if(rs.getString("comid")!=null) email_com_id = rs.getString("comid");
			if(rs.getString("comname")!=null) email_com_name = rs.getString("comname");
			...
			output_2 += "<img border=\"0\" src=\"images/mail/images/edit.gif\" /></a>";
			output_2 +=	"</td></tr></table>"; 				
				}
			 else{
			output_2 += "<div  id=\"maes_ref_"+email_id+"\"></div>";
			output_2 += " No Reference. <a href=\"#\" onclick=\"show('ref_edit_"+email_id+"');Ext.getCmp('mail_"+email_id+"').doLayout();return anchorPreventDefault(event);  \">";
			output_2 += " <img border=\"0\" src=\"images/mail/images/edit.gif\" /></a></td></tr></table>";
				}				
		output_2 += "<div  id=\"ref_mess_"+email_id+"\" class=\"hidden\"></div>";	
		refDtl = ref_dtl;
		//sql = "SELECT EMAIL_GROUP_ID,GROUP_NAME FROM EMAIL_GROUP";
		sql = MailSqlGenerator.getGroupDtl();
		rs = stmt.executeQuery(sql);
		while(rs.next()){
			email_group_id = rs.getString("componentId");
			email_group_name = rs.getString("groupname");
		...
		sql = MailSqlGenerator.getReferalDtl();
		rs = stmt.executeQuery(sql);
		while(rs.next()){
			ref_id = rs.getString("componentId");
			ref_dtl = rs.getString("refdtl");
		...
		sql = MailSqlGenerator.getUsersDtl();
		rs = stmt.executeQuery(sql);
		while(rs.next()){
			com_id = rs.getString("componentId");
		...
		*/	
		...
		sql = MailSqlGenerator.getHtmlMailDtlByEmailId(email_id);
		//System.out.println("\nSQL:"+sql);  
		rs = stmt.executeQuery(sql);
		if(rs.next()){
			contents_type = rs.getString("contenttype");
			clob_body=rs.getClob("content");
			body="";
			if(clob_body!=null)
			...
			sql = sql = MailSqlGenerator.getNoAttachmentMailDtlByEmailId(email_id);
			//System.out.println("\nSQL:"+sql);  
			rs = stmt.executeQuery(sql);
			if(rs.next()){
				contents_type = rs.getString("contenttype");
		...
		sql = MailSqlGenerator.getUpdateRecipientByEmailId(email_id);
