...
<%
	...
	try
	{
		...
		sql = MailSqlGenerator.getEmailSearchTotal(user_id,topic);
		...
		sql += " OR LOWER(EMAIL_DTL.CONTENTS) LIKE '"+topic+" %' OR LOWER(EMAIL_DTL.CONTENTS) LIKE '% "+topic+"' ))EMAIL,";
		sql += " ( SELECT MESSAGE_ID,STATUS FROM RECIPIENTS WHERE CONTACT_ID = '"+user_id+"' )REC WHERE EMAIL.ID = REC.MESSAGE_ID";
		sql = MailSqlGenerator.getEmailSearch(user_id,topic);
		System.out.println("sql::"+sql);
		//System.out.println("SQL:"+sql);
		rs = stmt.executeQuery(sql);
		while(rs.next()){
			 id = rs.getString("componentId");
			 ...
			 body_text = StringUtil.removeEscapeChar(body_text);
