 ...
 * @since mar 3, 2011
 */
public class EmailgroupSearchUtils
{
	protected static final Log log = LogFactory.getLog(EmailgroupSearchUtils.class);
	...
	public static String prepareSqlQuery( HttpServletRequest request)
	{
		...
		String projectSqlQuery = " SELECT a.componentId,a.groupname" +
				"  FROM emailgroup a";
projectSqlQuery += " WHERE a.groupname like '%" + searchQueryInput + "%'";
projectSqlQuery += " ORDER BY a.groupname" ;
	...
	public static ArrayList<String> getColumnHeader()
	{
		...
		columnHeader.add("label.emailgroup.groupName");
		/*
		columnHeader.add("label.emailgroup.name");		
		columnHeader.add("label.emailgroup.description"); 
		columnHeader.add("label.emailgroup.last.updatedby");
		 */ 
		return columnHeader;
	}
	...
	{
		String modifyURL = "emailgroupAction.csmp?method=promptModifyEmailgroup&componentId=";
